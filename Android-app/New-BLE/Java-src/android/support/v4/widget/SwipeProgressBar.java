    ...
    void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            if (!this.mRunning) {
                ...
                float clearRadius = ((float) (width / 2)) * INTERPOLATOR.getInterpolation((((float) ((now - this.mFinishTime) % 1000)) / Component.SLIDER_MIN_VALUE) / 100.0f);
            ...
            } else if (rawProgress >= 25.0f && rawProgress < Component.SLIDER_MAX_VALUE) {
                canvas.drawColor(this.mColor1);
            } else if (rawProgress < Component.SLIDER_MAX_VALUE || rawProgress >= 75.0f) {
            ...
            if (rawProgress >= 0.0f && rawProgress <= Component.SLIDER_MAX_VALUE) {
                canvas2 = canvas;
            ...
            if (rawProgress >= Component.SLIDER_MAX_VALUE && rawProgress <= 100.0f) {
                canvas2 = canvas;
                drawCircle(canvas2, (float) cx, (float) cy, this.mColor4, ((rawProgress - Component.SLIDER_MAX_VALUE) * 2.0f) / 100.0f);
            }
